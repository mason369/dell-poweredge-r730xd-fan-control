...
- 点击“开始轮询”或保存设置成功后会测试 iDRAC 连接、刷新一次 SDR 并启动持续 SDR 轮询；轮询中同一按钮显示为“取消轮询”，点击后停止后续传感器轮询 tick 并更新状态，不会伪装成又执行了一次连接测试。如果点击取消时已有一条 `ipmitool` 命令正在执行，该命令会按现有流程结束，应用不会再启动新的轮询 tick。默认间隔为 1 秒；同一时刻只允许一个 IPMI 操作，上一轮未完成时会跳过本次 tick，且不会启动新的 `ipmitool` 进程或 RMCP+ 会话。
...
- 所有实际 `ipmitool` 命令，包括 Dell 风扇控制 raw 命令（`raw 0x30 0x30 ...`），非零退出码都会立即失败并显示 stdout/stderr；应用不会自动重试、延迟重试或把失败记录成“将重试”。
...
- 总览交互图表位于 WebView2 中，滚轮事件会转交外层 WinUI `ScrollViewer` 执行页面滚动；转交时禁用额外滚动动画，鼠标停在图表上滚动时应与非 Web 看板一样跟手，不应卡住页面滚动。
...
- 切换温度曲线预设会启动软件自动轮询；每次 tick 读取 SDR、解析 CPU 温度、按曲线点位和当前平滑设置计算百分比，再发送全部风扇百分比命令。切换功耗曲线预设时，每次 tick 会读取 SDR、先检查 CPU 紧急温度，再用功耗读数计算百分比；如果本轮找不到功耗传感器，界面和日志会显示真实失败原因，并且不会下发风扇命令。
...
- Dell 风扇控制 raw 命令与轮询命令一样不做自动重试：`raw 0x30 0x30 ...` 非零退出码会立即失败并显示 stdout/stderr，读取传感器的 `sdr elist` 失败也不会重试或写入伪造历史点。
