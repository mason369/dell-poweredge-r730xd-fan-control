...
| 当前源码版本 | 应用版本 `1.1.3`；程序集、文件和 MSIX Identity 版本 `1.1.3.0`。打包版本以对应 Release 标签和 exe 文件版本为准。 |
| 当前 GitHub Release | 最新已发布版本为 `v1.1.3`，发布 zip 内的 exe/dll 文件版本为 `1.1.3.0`；源码提交、标签、GitHub Release 与打包二进制对应同一版本。不能只改 Release 标题或复用旧二进制文件。 |
...
- 切换温度曲线预设会启动软件自动轮询；每次 tick 读取 SDR、解析 CPU 温度并按曲线点位和平滑设置计算百分比。首轮强制执行进入手动模式和全部风扇百分比命令；首轮确认后的新目标只发送百分比命令。与上次成功值一致或与本模式最近失败值一致时不发送 raw 命令，后者会明确记录为已抑制的失败目标。功耗曲线同样先检查 CPU 紧急温度，再用本轮功耗读数计算；找不到功耗传感器时显示真实失败且不下发风扇命令，已经运行的策略会保留到下一次计划 tick。
- 手动全部风扇、单风扇、内置恢复手动预设、Dell 自动、总览/托盘还原戴尔出厂设置转速，以及停止自动都会清除当前曲线状态。用户主动命令会在等待 IPMI 锁前停止软件自动，旧的队列命令或后台 tick 之后只能记录自身结果，不能覆盖新模式和 LastRunning。删除正在运行的曲线预设、停止自动、首轮完整风扇命令失败、日志写入失败和紧急 Dell Auto 会停止软件自动。首轮确认后的后续百分比失败不切换模式、不发送 Dell Auto、不停止自动计时器，并抑制同一失败目标。完整命令序列的安全恢复若确认成功，会保存并显示 Dell Auto；恢复失败则把硬件模式保持为未确认。
...
软件恒温 tick 与传感器轮询、手动风扇命令和 Dell 自动还原共用同一把 IPMI 锁。启动软件自动、恢复上次运行的自动策略或切换曲线预设时，应用会等待当前 IPMI 命令结束，先执行首轮 tick，并强制执行一次“进入手动模式 + 下发当前 SDR 计算出的全部风扇百分比”；首轮成功后才启动后台计时器、写入 LastRunning 状态，并把横幅、模式卡片和活动预设标为对应自动模式。如果首轮失败，包括 SDR 读取超时、找不到功耗传感器、raw 风扇命令失败或日志写入失败，计时器不会启动，界面和 JSONL 会显示真实阶段与根因，不会把自动策略标成已运行。如果在建立新的风扇控制意图前，设置保存或输入校验已经失败，应用只显示该失败并保留原模式，不会改成手动。进入手动模式成功但首个百分比失败时只执行一次 Dell 自动安全恢复，不重试失败百分比；恢复成功后显示并保存 Dell Auto，恢复也失败时明确保留“BMC 模式未确认”的风险。

首轮目标确认后，后续自动 tick 改变百分比时只发送 `raw 0x30 0x30 0x02 0xff <percent>`，不会重复发送进入手动模式命令，也不会因为该百分比失败而切换模式或执行 Dell 自动恢复。失败百分比会连同最后一次确认成功的百分比写入日志，并标记 `modeChangeAttempted=False`、`retrySuppressed=True`；当前自动策略、模式摘要、计时器和 LastRunning 状态保持不变。以后 tick 仍会真实读取 SDR、刷新传感器和图表，但同一自动模式计算出同一个失败百分比时只记录 `SkipPreviouslyFailedFanPercent`，不再发送 raw 命令；计算结果变成其他百分比后才允许尝试新的目标。成功下发新目标或用户发起手动、预设、Dell Auto、停止自动等新控制意图时，会清除失败目标缓存。旧 tick 完成或失败前如果已有新用户意图，旧 tick 只能记录自身结果，不能覆盖新模式、活动预设或 LastRunning 状态。

自动策略运行期间，普通传感器轮询不会再单独启动 `sdr elist`，避免同一周期内由两个后台定时器重复建立 RMCP+ 会话；自动 tick 的 SDR 结果负责刷新传感器、看板、图表和历史点。后台 tick 遇到 IPMI 忙会跳过，不排队启动新的 `ipmitool`。目标百分比与同一自动模式下上次成功值一致时记录 `SkipUnchangedFanPercent` 并跳过 raw 命令，但仍保留本轮真实 SDR 更新。已经运行的自动策略如果在目标计算前遇到 SDR 读取、CPU/功耗读数解析或正常风扇命令后的确认刷新失败，会显示并记录真实失败，同时保留当前自动策略和 LastRunning，下一次计划 tick 再做一次真实 SDR 读取。手动风扇、手动预设、Dell Auto 和总览/托盘还原属于新的用户意图，会在等待 IPMI 锁前停止软件自动，再执行用户请求。如果紧急阈值触发且 Dell Auto 命令已经成功，软件自动会停止并显示、保存 Dell Auto；即使随后的传感器确认刷新失败，也不会错误显示为手动模式。
...
- 达到紧急自动阈值时，无论当前使用全局线性策略还是曲线预设，都会优先发送 Dell 自动模式命令；命令确认成功后会停止计时器、清空自动模式状态，并把 Dell 自动保存为当前运行模式。其后的传感器刷新若失败，会显示刷新失败，但不会把已经确认的 Dell Auto 改成手动模式。
...
- Dell 风扇控制 raw 命令与用户触发的单次传感器刷新不重试同一条失败命令：`raw 0x30 0x30 ...` 非零退出码会立即失败并显示 stdout/stderr，失败的 `sdr elist` 也不会写入无真实 SDR 来源的历史点；后台轮询失败会持续追加可见重连。已经运行的软件自动或曲线自动在计算目标前遇到 SDR/读数处理失败时，会保留所选自动策略并在下一次计划 tick 重新读取。后续自动目标 raw 失败时不切换模式，也不重复失败目标；只有新的计算百分比才会再次下发。
