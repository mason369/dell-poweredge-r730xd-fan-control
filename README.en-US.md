...
| Current source version | App version `1.1.3`; assembly, file, and MSIX Identity version `1.1.3.0`. Use the matching Release tag and executable file version for a packaged download. |
| Current GitHub Release | The latest published release is `v1.1.3`; its zip contains exe/dll file version `1.1.3.0`, and the source commit, tag, GitHub Release, and packaged binaries identify the same version. Do not update only the Release title or reuse older binaries. |
...
- After a manual preset, Dell Auto, temperature curve, power curve, or smart temperature policy starts successfully, the running state is written to `%LocalAppData%\DellR730xdFanControlCenter\settings.json`. On the next app launch, after a real connection/start-polling sequence succeeds, the app re-executes the saved preset or automatic policy. Restore failures show the real error instead of only marking the UI as running. Smart auto and curve auto mark the hero, mode card, and active preset as automatic only after the first real tick succeeds. Start failure and Stop Auto clear those automatic-mode states. Emergency Dell Auto leaves Dell Auto as the saved mode. If a percentage raw command fails after manual-mode entry and the dedicated Dell automatic recovery succeeds, the requested tick remains failed, the software-auto timer stops, and the UI plus LastRunning move to Dell Auto instead of Manual; if that recovery also fails, software auto still stops and the app reports that hardware mode is unconfirmed. If an already-running timer fails to read SDR before calculating a target, or a normal raw fan command succeeds but its immediate confirmation `sdr elist` refresh fails, the app shows and logs the failed tick while preserving the selected automatic policy and LastRunning state; the next scheduled tick performs another real SDR read. The same failures during the first startup tick still prevent the timer from starting, and a confirmation-refresh failure after emergency Dell Auto does not preserve software-auto state.
...
- The Overview interactive chart runs in WebView2, but wheel input is normalized in the browser and immediately forwarded to the outer WinUI `ScrollViewer`. Native content and chart input use the same cumulative target and animated `ChangeView` path. Rapid wheel events add to the preceding animation target instead of recalculating from an unfinished intermediate offset, so earlier distance is not lost; the chart no longer adds a second animation-frame delay before posting the event. Touchpad-style high-frequency small deltas retain their pixel distance. ECharts internal wheel zoom remains disabled, and history ranges are adjusted with the bottom slider.
...
- Temperature, Fan RPM, Power & Health, and the detailed sensor list disable their internal vertical scrollbars, leaving the outer WinUI `ScrollViewer` as the single page scroll owner. Native content and the chart WebView feed wheel input into the same normalized distance, cumulative target, and animated `ChangeView` path. ECharts still uses the bottom slider for history range selection and does not consume page-wheel input.
...
- Switching to a temperature curve starts software auto polling. Each tick reads SDR, parses CPU temperature, and evaluates the curve and smooth setting. The first tick forces manual-mode entry plus the all-fan percentage; later new targets send only the percentage command. A match with the last confirmed value or the most recent rejected value for that mode sends no raw command, with the latter recorded as a suppressed failed target. Power curves also check emergency CPU temperature before using current power. A missing power sensor is shown as a real failure without a fan command, and an already-running policy remains active for the next scheduled tick.
- Manual all-fan control, individual control, Restore Manual, Dell Auto, Overview/tray restore, and Stop Auto clear current curve state. User commands stop software auto before waiting for the IPMI lock; an older queued command or background tick may later record its own result but cannot overwrite the newer mode or LastRunning. Deleting the running curve, Stop Auto, first-tick full fan-command failure, runtime-log failure, and emergency Dell Auto stop software auto. A later percentage failure after the first confirmed target does not change mode, send Dell Auto, or stop the timer, and the same target is suppressed. Confirmed safety recovery for the full sequence persists and displays Dell Auto; failed recovery leaves hardware mode unconfirmed.
...
Smart auto ticks share one IPMI lock with sensor polling, manual fan commands, and Dell automatic restore. Starting smart auto, restoring a saved automatic policy, or switching to a curve first runs one real tick. That first tick forces the full `enter manual mode + set all-fan percentage` sequence for the target computed from current SDR data. Only after it succeeds does the app start the timer, write LastRunning state, and mark the hero, mode card, and active preset as automatic. A first-tick SDR, power-sensor, raw-command, or runtime-log failure remains visible and does not mark the policy as running. If settings persistence or input validation fails before a new fan-control intent is created, the app only displays that failure and preserves the previous mode instead of showing Manual. If manual-mode entry succeeded but the first percentage failed, the app sends one Dell Auto safety-recovery command and never retries the failed percentage; confirmed recovery persists and displays Dell Auto, while failed recovery leaves BMC mode explicitly unconfirmed.

After the first target has been confirmed, a later automatic target change sends only `raw 0x30 0x30 0x02 0xff <percent>`; it does not send the manual-mode command again and does not switch mode or invoke Dell Auto recovery when that percentage fails. The failure log includes the last confirmed percentage plus `modeChangeAttempted=False` and `retrySuppressed=True`, while the current automatic policy, mode summary, timer, and LastRunning state remain active. Later ticks still perform real SDR reads and refresh sensors and charts, but the same failed percentage for the same automatic mode is recorded as `SkipPreviouslyFailedFanPercent` without another raw command. A different computed percentage may be attempted. A later success or a new user intent—manual control, preset, Dell Auto, or Stop Auto—clears the failed-target cache. If an old tick completes or fails after a newer user intent exists, it may record its own outcome but cannot overwrite the newer mode, active preset, or LastRunning state.

While automatic control is running, regular polling does not start a second `sdr elist`; the automatic tick owns sensor, dashboard, chart, and history refresh. A busy IPMI lock skips the background tick without queuing another `ipmitool`. An unchanged confirmed target records `SkipUnchangedFanPercent` and skips the raw command while retaining the real SDR refresh. A running automatic policy also remains active when a failure happens before target computation during SDR reading, CPU/power parsing, or the confirmation refresh after a normal fan command; the failure is shown and logged, and the next scheduled tick performs another real SDR read. Manual control, manual presets, Dell Auto, and Overview/tray restore are new user intents that stop software auto before waiting for the IPMI lock. If the emergency threshold is reached and the Dell Auto command succeeds, software auto stops and Dell Auto is displayed and persisted; a later confirmation-refresh failure is reported without incorrectly changing the mode to Manual.
...
- At or above the emergency auto threshold, Dell Auto is restored first whether the app is using the global linear policy or a curve preset. Once that command is confirmed, the timer stops, automatic-mode state is cleared, and Dell Auto is persisted as the running mode. A later sensor-refresh failure is reported but does not change the confirmed Dell Auto state to Manual.
