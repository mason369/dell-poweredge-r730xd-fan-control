...
| Current source version | App version `1.1.2`; assembly, file, and MSIX Identity version `1.1.2.0`. Use the matching Release tag and executable file version for a packaged download. |
| Current GitHub Release | The latest published release remains `v1.1.0`, whose zip contains exe/dll file version `1.1.0.0`; source `1.1.2` becomes a release only after its matching committed tag runs GitHub Actions. Do not update only the Release title or reuse older binaries. |
