...
连接成功后，应用会启动持续传感器轮询。默认间隔为 1 秒，保存时允许 1 秒及以上的值。需要注意：

- 1 秒是轮询 tick 的发起频率，不是 SDR 数据的实时流式返回速度；实际耗时以总览页和运行日志为准。
- 如果上一次 SDR 读取尚未完成，本次 tick 会跳过；跳过日志写入页面日志和运行 JSONL 日志，不会打开或覆盖顶部 InfoBar。
- 如果其他 IPMI 命令正在执行，本次 tick 会跳过，避免并发命令冲突；同一段忙碌期间只记录第一条跳过日志。
