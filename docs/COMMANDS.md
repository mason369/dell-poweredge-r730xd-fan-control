...
脚本会先校验所有输出路径都位于仓库根目录下，再清理专用 `artifacts/release/` 输出目录；旧下载包、旧解压目录或旧 `.exe.WebView2` 数据不会留在新 zip 旁边。随后执行 `tools\Publish-UnpackagedExe.ps1` 生成 `artifacts/exe/win-x64/`；该脚本同样会先清理专用 exe 输出目录，避免旧文件进入压缩包。完成后创建 `artifacts/release/DellR730xdFanControlCenter-win-x64.zip`，并把 zip 解压到临时目录，检查 `DellR730xdFanControlCenter.exe`、`Microsoft.WindowsAppRuntime.dll`、`Microsoft.ui.xaml.dll`、`DellR730xdFanControlCenter.pri`、`LICENSE`、`THIRD_PARTY_NOTICES.md`、`Assets/Charts/dashboard.html`、`Assets/Charts/echarts.min.js`、ECharts 许可证/NOTICE、`BundledTools/ipmitool/ipmitool.exe` 和 `BundledTools/ipmitool/LICENSES/**`。该 zip 是 unsigned unpackaged 下载物，不走 MSIX 安装；如果解压内容包含 `.msix`、`.pfx`、`.cer`、`AppxManifest.xml`、`Package.appxmanifest` 或 `DellR730xdFanControlCenter.exe.WebView2`，脚本会失败停止。`ExeOutputDirectory`、`ReleaseOutputDirectory`、`ZipName`、校验目录、signed-MSIX 输出目录和相关递归清理目录都必须解析到仓库根目录下；传入 `..` 或绝对越界路径会在创建、压缩、签名或删除前直接失败。本机验证下载后的 zip 能否启动时运行：
...
- `Operation/SmartAutoPolicyTick`：每次软件自动策略 tick。记录温度阈值、CPU 温度、曲线预设类型、功耗曲线使用的 `powerWatts`、计算出的风扇百分比，或达到紧急阈值后的 Dell 自动动作。首轮完整命令使用 `action = SetAllFansManualSpeed`；首轮确认后的百分比变化使用 `action = SetFanSpeedInConfirmedManualMode` 和 `modeChangeAttempted=False`。目标与上次成功值一致时记录 `SkipUnchangedFanPercent`；目标与本模式先前失败值一致时记录 `SkipPreviouslyFailedFanPercent` 和 `retrySuppressed=True`。后续目标失败的 `Failed` 记录会保留 `cpuTemperatureCelsius`、`fanPercent`、最后一次确认成功百分比和 `retrySuppressed=True`，功耗曲线还会保留 `powerWatts`。
...
| 进入手动风扇模式 | `raw 0x30 0x30 0x01 0x00` | 设置全部风扇、手动预设、内置“默认/恢复手动 10%”预设、软件恒温策略 | 用户手动命令和自动策略首轮在设置百分比前发送。自动策略首轮目标确认后，后续百分比变化不再重复发送该命令。总览和托盘的“还原戴尔出厂设置转速”也不发送该命令。 |
| 设置全部风扇百分比 | `raw 0x30 0x30 0x02 0xff <percent-hex>` | 全部风扇控制、手动预设、托盘全部风扇菜单、软件恒温/曲线自动 | `0xff` 表示全部风扇。自动策略后续目标只发送本命令；同一失败目标会被抑制，不重复下发。 |
| 设置单风扇百分比 | `raw 0x30 0x30 0x02 <fan-target-selector> <percent-hex>` | 单风扇控制 | 默认关闭。`<fan-target-selector>` 是风扇目标编号，不是转速；启用前必须确认固件映射。 |
| 恢复 Dell 自动模式 | `raw 0x30 0x30 0x01 0x01` | 总览/托盘“还原戴尔出厂设置转速”、Dell 自动预设、紧急温度保护、完整手动模式/百分比序列失败后的一次安全恢复 | 把风扇控制权交回 iDRAC/BMC 固件策略。用户主动恢复会在等待 IPMI 锁前停止软件自动；紧急保护和确认成功的安全恢复也会停止软件自动并保存 Dell 自动状态。首轮已确认后的单独百分比失败不会触发此命令。 |
...
自动策略首轮使用完整“进入手动模式 + 百分比”序列。如果进入手动模式已经成功而首个百分比失败，应用只发送一次 Dell 自动恢复命令；恢复确认成功后，百分比请求仍保持失败，但软件自动计时器会停止，界面和 LastRunning 模式改为 Dell 自动。恢复也失败时会同时显示两次错误，并明确标出硬件模式未确认。

首轮目标确认后，后续自动目标只发送 `raw 0x30 0x30 0x02 0xff <percent-hex>`。该命令失败时，`Operation/SmartAutoPolicyTick` 会记录 CPU 温度、功耗、失败百分比、最后一次确认成功百分比、`modeChangeAttempted=False` 和 `retrySuppressed=True`；软件自动计时器、当前自动模式和 LastRunning 保持不变，也不发送 Dell Auto。同一模式再次计算出相同失败百分比时只记录 `SkipPreviouslyFailedFanPercent`，不会启动 `ipmitool`；计算结果变化后才允许尝试新的百分比。旧 tick 在新用户控制意图之后完成时，不得覆盖新模式或持久化状态。

`mc info`、`sdr elist`、raw 命令、参数校验失败、缺少 `ipmitool.exe` 和命令超时都不会重试同一条失败命令、延迟重试或写入“将重试”记录。周期性自动 tick 仍会继续读取新的 SDR，但被抑制的失败百分比不会再次下发。传感器后台轮询失败会先停止本轮轮询并显示根因，然后执行一次真实重连流程（`mc info` + `sdr elist`）；重连成功才恢复持续轮询，重连失败继续显示真实错误并保持断开，不会写入伪造图表历史点。
...
所有会执行 IPMI 的托盘动作都共用应用内同一把 IPMI 锁。已有 IPMI 命令运行时，用户主动触发的托盘命令会等待当前命令结束后继续执行，不会启动并发 `ipmitool` 进程；切换完成前，界面不会显示成功状态。后台轮询、软件自动和曲线自动 tick 仍然在 IPMI 忙时跳过；同一段忙碌期间只把第一条跳过原因写入页面日志和运行日志。
...
该命令让 iDRAC/BMC 固件自动策略接管风扇。总览快速操作和托盘菜单中的“还原戴尔出厂设置转速”会执行这一条命令。这些用户入口会先停止软件自动计时器并清除旧的自动目标和 LastRunning 自动状态，再等待当前 IPMI 命令结束，避免旧 tick 覆盖新的用户意图。命令成功后，界面和 LastRunning 显示 Dell 自动；失败时显示真实 IPMI 错误，不会显示恢复成功状态。
...
- 达到或超过紧急自动阈值：发送 Dell 自动模式命令；该命令成功后停止软件自动计时器、清空曲线/软件自动状态，并把 Dell 自动保存为当前运行模式。随后确认刷新失败会报告刷新失败，但不会改写已确认的 Dell Auto 模式。

软件自动 tick 与传感器轮询和用户触发的风扇命令互斥。用户主动启动软件自动或切换曲线预设时，应用会等待当前 IPMI 命令完成，再执行首轮 tick；首轮成功后才启动后台计时器，首轮失败会显示真实错误且不会启动后台计时器。若设置保存或输入校验在建立新控制意图前失败，只显示真实失败并保留原模式，不会改成手动。后台 tick 触发时如果上一轮自动策略仍在运行，或已有 IPMI 命令占用锁，本次自动策略周期会跳过；同一段忙碌期间只记录第一条跳过日志，不会打开或覆盖顶部 InfoBar，也不会启动新的 `ipmitool` 进程或 RMCP+ 会话。
已经运行的自动策略若在目标计算前读取/处理 SDR 失败，或正常 raw 风扇命令已经成功、但紧随其后的确认刷新 `sdr elist` 失败，本轮 `Operation/SmartAutoPolicyTick` 会按真实失败写入日志并在顶部显示错误；自动计时器、当前曲线/软件恒温摘要和 LastRunning 状态保持不变，下一次计划 tick 会再次执行真实 SDR 读取。该规则不把失败记录为成功，也不重发刚才已经成功的 raw 命令。首轮启动遇到同类失败仍会停止启动且不写入运行状态。确认成功的 Dell 自动安全恢复和紧急 Dell 自动会保存 Dell 自动；紧急 Dell Auto 后的确认刷新失败只报告刷新失败，不会错误显示手动。恢复失败或未经确认时不会显示 Dell 自动成功。
后台 tick 成功读取 SDR 并计算出目标百分比后，会先与同一自动模式最近一次成功值和最近一次失败值比较。与成功值一致时记录 `SkipUnchangedFanPercent`；与失败值一致时记录 `SkipPreviouslyFailedFanPercent`。两者都只更新传感器、看板、图表和历史点，不执行 raw 风扇命令。不同的新目标使用无模式切换的百分比命令；失败时保留自动策略并记住失败目标。切换自动模式、手动全部风扇、单风扇、Dell 自动或停止自动会清除成功/失败缓存，下一次自动接管会重新执行完整首轮命令。
...
- 达到紧急自动阈值时，曲线计算结果不会继续下发，应用先发送 Dell 自动模式命令；命令成功后停止软件自动计时器，并把 Dell 自动保存为当前运行模式。随后确认刷新失败会显示真实失败，但不会把已确认的 Dell Auto 改成手动。

如果未达到紧急阈值，自动策略首轮或尚未确认手动模式时发送：
...
首轮目标确认后，计算出不同的新百分比时只发送第二条百分比命令；百分比不变或等于本模式已记录的失败目标时不发送 raw 命令。
...
- 如果轮询命令失败，应用会停止当前轮询、标记断开并显示失败原因，然后释放 IPMI 锁并执行一次真实重连流程（`mc info` + `sdr elist`）。重连成功才恢复持续轮询；重连失败继续显示真实错误并保持断开，不会静默降级，界面也不会显示成功状态。
...
- 下次打开软件后，应用仍会先执行“开始轮询”流程：`mc info` 成功、一次 `sdr elist` 成功并启动轮询后，才会重新执行上次保存的预设或软件恒温策略。连接或首轮读取失败时，界面不会显示恢复成功状态。
...
iDRAC 返回完整 SDR 可能需要数秒到十几秒。过低轮询会持续建立 IPMI v2/RMCP+ 会话，可能导致 `Unable to establish IPMI v2 / RMCP+ session`。软件自动或曲线自动运行时，普通轮询不会再额外读取 SDR，避免与自动策略 tick 重复建立会话。在设置页“应用设置”中把轮询间隔提高到界面推荐值或更高。后台轮询失败会触发一次可见的真实重连，但失败命令本身不会被重试，应用也不会把失败轮询标记为成功。
