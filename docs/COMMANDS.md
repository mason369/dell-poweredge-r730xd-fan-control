...
| 恢复 Dell 自动模式 | `raw 0x30 0x30 0x01 0x01` | 总览/托盘“还原戴尔出厂设置转速”、Dell 自动预设、紧急温度保护、百分比命令失败后的一次安全恢复 | 把风扇控制权交回 iDRAC/BMC 固件策略。用户主动恢复会在等待 IPMI 锁前停止软件自动；紧急保护和确认成功的安全恢复也会停止软件自动并保存 Dell 自动状态。 |
...
软件自动或曲线自动已经完成 SDR 读取并计算出目标百分比后，如果 `raw 0x30 0x30 0x02 ...` 失败，失败的 `Operation/SmartAutoPolicyTick` 记录会带上本轮 CPU 温度、功耗读数、目标百分比和动作名。该记录用于排查，不代表命令成功；应用仍会显示真实失败并停止该自动策略周期。如果进入手动模式已经成功，应用只发送一次 Dell 自动恢复命令。恢复确认成功后，百分比请求仍保持失败，但软件自动计时器会停止，界面和持久化 LastRunning 模式改为 Dell 自动，而不是手动；恢复也失败时会同时显示两次错误，并明确标出硬件模式未确认。
...
- 达到或超过紧急自动阈值：发送 Dell 自动模式命令；命令及其确认刷新成功后停止软件自动计时器、清空曲线/软件自动状态，并把 Dell 自动保存为当前运行模式。
...
已经运行的自动策略若在计算目标前读取 SDR 失败，或正常 raw 风扇命令已经成功、但紧随其后的确认刷新 `sdr elist` 失败，本轮 `Operation/SmartAutoPolicyTick` 会按真实失败写入日志并在顶部显示错误；自动计时器、当前曲线/软件恒温摘要和 LastRunning 状态保持不变，下一次计划 tick 会再次执行真实 SDR 读取。该规则不把失败记录为成功，也不重发刚才已经成功的 raw 命令。首轮启动遇到同类失败仍会停止启动且不写入运行状态。raw 风扇命令失败和日志写入失败会停止软件自动；确认成功的 Dell 自动安全恢复和紧急 Dell 自动会保存 Dell 自动，而不是手动；恢复失败或未经确认时不会显示 Dell 自动成功。
...
- 达到紧急自动阈值时，曲线计算结果不会继续下发，应用先发送 Dell 自动模式命令；命令及确认刷新成功后停止软件自动计时器，并把 Dell 自动保存为当前运行模式。
