...
连接成功后，应用会启动持续传感器轮询。最短间隔为 1 秒。总览页会显示最后轮询时间和本次读取耗时。如果上一次 SDR 读取尚未完成，或其他 IPMI 命令正在执行，下一次 tick 会跳过并给出提示，避免同时发起多条 IPMI 命令。若单次读取耗时超过设置的轮询间隔，软件会通过状态栏和日志提示 BMC 网络或 iDRAC 响应存在波动。
