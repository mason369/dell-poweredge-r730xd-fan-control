...
This command lets the iDRAC/BMC firmware policy take over fan control. Overview Quick Actions and the tray menu item "Restore Dell factory fan speed" execute this command. On success, the UI shows Dell automatic mode; on failure, the real IPMI error is shown and the UI does not show a successful restoration state. The command does not stop an already running software auto timer; use "Stop auto" when firmware control must remain in effect.
