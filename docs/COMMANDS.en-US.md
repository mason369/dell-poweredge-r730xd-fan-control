...
- `Operation/SmartAutoPolicyTick`: each software auto policy tick. Records temperature thresholds, CPU temperature, curve type, `powerWatts` for power curves, computed fan percent, or the emergency Dell Auto action. The full first-tick sequence uses `action = SetAllFansManualSpeed`; later target changes use `action = SetFanSpeedInConfirmedManualMode` plus `modeChangeAttempted=False`. An unchanged confirmed target records `SkipUnchangedFanPercent`; a target matching the rejected percentage for the same mode records `SkipPreviouslyFailedFanPercent` and `retrySuppressed=True`. A failed later target retains `cpuTemperatureCelsius`, `fanPercent`, the last confirmed percentage, `retrySuppressed=True`, and `powerWatts` when applicable.
...
| Enter manual fan mode | `raw 0x30 0x30 0x01 0x00` | All-fan control, manual presets, built-in Default/Restore Manual 10% preset, smart auto policy | User manual commands and the first automatic tick send this before the percentage. After the first automatic target is confirmed, later percentage changes do not repeat it. Overview/tray "Restore Dell factory fan speed" does not send it. |
| Set all-fan percentage | `raw 0x30 0x30 0x02 0xff <percent-hex>` | All-fan control, manual presets, tray all-fan menu, smart/curve auto | `0xff` targets all fans. Later automatic targets send only this command; the same rejected target is suppressed instead of sent again. |
...
| Restore Dell automatic mode | `raw 0x30 0x30 0x01 0x01` | Overview/tray "Restore Dell factory fan speed", Dell Auto preset, emergency protection, one-shot recovery after a full manual-mode/percentage sequence failure | Hands fan control back to iDRAC/BMC firmware policy. User restore stops software auto before waiting for the IPMI lock; emergency and confirmed safety recovery also stop software auto and persist Dell Auto. A later percentage-only automatic failure does not send this command. |
...
The first automatic tick uses the full `enter manual mode + percentage` sequence. If manual-mode entry succeeds but the first percentage fails, the app sends exactly one Dell automatic recovery command. Confirmed recovery keeps the percentage request failed, stops software auto, and sets the visible and LastRunning mode to Dell Auto. If recovery also fails, both errors remain visible and hardware mode is explicitly unconfirmed.

After the first target is confirmed, later automatic targets send only `raw 0x30 0x30 0x02 0xff <percent-hex>`. A failure records CPU temperature, power, failed percentage, last confirmed percentage, `modeChangeAttempted=False`, and `retrySuppressed=True`; the timer, automatic mode, and LastRunning remain active, and Dell Auto is not sent. Computing the same failed percentage again records `SkipPreviouslyFailedFanPercent` without starting `ipmitool`; a changed percentage may be attempted. An old tick that completes after a newer user intent cannot overwrite the new mode or persisted state.

`mc info`, `sdr elist`, raw commands, argument validation failures, missing `ipmitool.exe`, and command timeouts do not retry the same failed command, delay and retry, or write "retrying" records. Periodic automatic ticks still read new SDR data, but a suppressed rejected percentage is not sent again. Background polling failures first stop the current poll and show the root cause, then run one real reconnect flow (`mc info` + `sdr elist`); persistent polling resumes only after success. Reconnect failure remains visible and disconnected without fabricated chart history.
...
- At or above the emergency auto threshold: send the Dell automatic mode command; once that command succeeds, stop software auto, clear curve/smart-auto state, and save Dell Auto. A later confirmation-refresh failure reports the refresh failure without overwriting confirmed Dell Auto.

Smart auto ticks are mutually exclusive with sensor polling and user-triggered fan commands. When the user starts smart auto or switches to a curve preset, the app waits for the current IPMI command to finish, then runs the first tick before starting the background timer; if that first tick fails, the timer is not started and the UI/logs show the real failure reason. If settings persistence or input validation fails before a new control intent is created, only the real failure is shown and the previous mode is preserved instead of changing to Manual. If a later background tick fires while the previous auto-policy tick is still running or another IPMI command holds the lock, that auto-policy cycle is skipped; only the first skipped tick in the same busy period is logged, the top InfoBar is not opened or overwritten, and no new `ipmitool` process or RMCP+ session is started.
If an already-running policy fails while reading or processing SDR before a target, or after a normal raw command succeeds but its confirmation `sdr elist` refresh fails, `Operation/SmartAutoPolicyTick` records the real failure and the top error is shown. The timer, automatic summary, and LastRunning remain active; the next scheduled tick performs another real SDR read and does not resend the command that already succeeded. The same failures on the first startup tick still abort startup without persisting running state. Confirmed Dell automatic safety recovery and emergency Dell Auto persist Dell Auto; a confirmation-refresh failure after a successful emergency Dell Auto command reports the refresh failure without showing Manual. Failed or unconfirmed recovery does not claim Dell Auto success.
After SDR produces a target, the tick compares it with both the last confirmed and last rejected percentages for the same automatic mode. A confirmed match records `SkipUnchangedFanPercent`; a rejected match records `SkipPreviouslyFailedFanPercent`. Both update sensors, boards, charts, and history without a raw command. A different target uses the percentage-only command and remains automatic if that target fails. Switching automatic modes, manual all-fan/individual control, Dell Auto, or Stop Auto clears both caches so the next automatic takeover runs its full first-tick sequence.
...
- At or above the emergency auto threshold, the curve result is not sent; the app sends Dell automatic mode first. Once that command succeeds, software auto stops and Dell Auto is saved. A later confirmation-refresh failure is reported without changing confirmed Dell Auto to Manual.

If the emergency threshold is not reached, the first automatic tick or an unconfirmed manual state sends:
...
After the first target is confirmed, a different computed percentage sends only the second percentage command. An unchanged percentage or the rejected target already recorded for that mode sends no raw command.
