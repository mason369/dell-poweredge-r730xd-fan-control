...
- An already-running automatic policy shows and logs a real failure but preserves its timer, current automatic mode, and LastRunning state when the initial SDR read fails or the confirmation refresh after a successful normal raw fan command fails; the next scheduled tick reads SDR again. First-tick startup failure, raw-command failure, and runtime-log write failure stop software auto. If manual-mode entry succeeded and the percentage command failed, a confirmed one-shot Dell automatic recovery keeps the tick failed but persists/shows Dell Auto instead of Manual; failed recovery leaves hardware mode unconfirmed. Emergency Dell Auto also stops software auto and persists Dell Auto.
...
- At the emergency auto threshold, the app sends Dell automatic mode instead of continuing to send curve percentages. After that command and its confirmation refresh succeed, it stops the software-auto timer, clears curve/smart-auto state, and saves Dell Auto as the current running mode. A failed confirmation refresh also stops and clears software-auto state, but shows the real failure and does not mark unconfirmed Dell Auto state as successful.
...
- Manual all-fan control, individual fan control, the built-in Restore Manual preset, Dell Auto, Overview/tray Restore Dell factory fan speed, and Stop Auto are new user control intents. They stop the software-auto timer before waiting for the IPMI lock and clear the active curve, LastRunning automatic state, and automatic target cache. Deleting the running curve preset, raw fan-command failure, runtime-log write failure, and emergency Dell Auto also stop software auto. Confirmed Dell automatic recovery persists/shows Dell Auto while keeping the original percentage request failed; failed recovery does not claim a confirmed hardware mode.
