...
- An already-running policy shows and logs a real failure but preserves its timer, automatic mode, and LastRunning when SDR reading or CPU/power parsing fails before target computation, or the confirmation refresh after a normal raw command fails; the next scheduled tick reads SDR again. A settings-persistence or input-validation failure before a new control intent exists only shows the failure and preserves the previous mode. The first tick uses the full `enter manual mode + percentage` sequence. If manual entry succeeds but the percentage fails, one Dell Auto recovery is sent and the percentage is not retried; confirmed recovery persists/shows Dell Auto, while failed recovery leaves hardware mode unconfirmed. After the first target is confirmed, later targets send only the percentage raw command; a failure does not change mode or send Dell Auto, retains the policy, and suppresses that rejected percentage. A successful emergency Dell Auto command stops software auto and persists Dell Auto even if its later confirmation refresh fails.
...
- WebView2 interactive charts immediately forward normalized wheel distance to the outer WinUI scroll container. Chart and native content share one cumulative target and WinUI `ChangeView` animation path, so rapid input adds to the preceding animation target instead of losing distance by reading an in-flight intermediate offset. Touchpad-style high-frequency small deltas retain their pixel distance; ECharts internal wheel zoom is disabled and history ranges remain on the bottom slider.
...
- Each tick reads `sdr elist`. Temperature curves parse CPU temperature and evaluate their points plus `SmoothCurve`; power curves first parse CPU temperature and check the emergency threshold, then evaluate the current SDR power reading whose unit contains `Watts` or whose key contains `Pwr Consumption`. Regular polling does not add a second SDR read while auto runs; the tick refreshes sensors, charts, and history. The first tick forces the manual-mode and all-fan percentage commands. Later new percentages send only the all-fan percentage command. A match with the last confirmed value records `SkipUnchangedFanPercent`; a match with the last rejected value records `SkipPreviouslyFailedFanPercent`. Neither match starts another raw command.
...
- Non-zero exits from Dell fan-control raw commands (`raw 0x30 0x30 ...`) fail immediately; every actual `ipmitool` child writes one real result without `attempt`, `maxAttempts`, or `willRetry`, and stdout/stderr is shown. A post-first-tick automatic percentage failure records `modeChangeAttempted=False`, `retrySuppressed=True`, and the last confirmed value; later ticks still read SDR but do not start `ipmitool` for the same rejected target. `sdr elist` polling failures do not retry the same failed command or write fabricated history; only background polling failures append a visible real reconnect.
...
- At the emergency auto threshold, the app sends Dell automatic mode instead of continuing curve percentages. After that command succeeds, it stops software auto, clears curve/smart-auto state, and saves Dell Auto as the current running mode. A later confirmation-refresh failure is shown but does not overwrite the Dell Auto state already confirmed by the command.
...
- Manual all-fan control, individual fan control, the built-in Restore Manual preset, Dell Auto, Overview/tray Restore Dell factory fan speed, and Stop Auto are new user intents. They stop software auto before waiting for the IPMI lock and clear the active curve, LastRunning automatic state, and confirmed/rejected target caches. An old automatic tick that later completes or fails may log its own result but cannot clear the newer mode or state. Deleting the running curve, first-tick full fan-command failure, runtime-log failure, and emergency Dell Auto stop software auto; a later percentage failure after the first confirmed target retains software auto and suppresses that target. Confirmed Dell recovery persists/shows Dell Auto while the original percentage request remains failed; failed recovery does not claim a confirmed hardware mode.
