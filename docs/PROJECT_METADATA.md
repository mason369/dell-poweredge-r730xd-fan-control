...
- 若发布 GitHub Actions 或 Release 下载 zip，使用 `tools/Publish-ReleaseZip.ps1` 或 `.github/workflows/release.yml`，并确认 zip 解压后包含 exe、`Microsoft.WindowsAppRuntime.dll`、`Microsoft.ui.xaml.dll`、`DellR730xdFanControlCenter.pri`、图表资产和内置 `ipmitool.exe`；本机可用 `tools/Publish-ReleaseZip.ps1 -VerifyLaunch` 验证解压后的 zip 能启动窗口。GitHub Actions 手动触发只上传 workflow artifact，推送 `v*` tag 时才上传 GitHub Release 资产，并用 `gh release upload --clobber` 允许同一 tag 重跑覆盖同名 zip。
