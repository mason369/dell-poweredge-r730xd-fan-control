...
- WebView2 交互图表会把滚轮转交外层 WinUI 滚动容器；离散鼠标滚轮使用 WinUI 平台滚动动画，触控板/高频小幅滚动按合并后的增量直接移动。ECharts 内部滚轮缩放关闭，历史范围仍通过底部滑块调整，鼠标停在图表上不会卡住页面滚动。
...
- 切换曲线预设会启动软件自动轮询。
- 每次 tick 读取 `sdr elist`。温度曲线解析 CPU 温度并按曲线点和 `SmoothCurve` 设置计算百分比；功耗曲线先解析 CPU 温度并检查紧急自动阈值，再使用本轮 SDR 中单位包含 `Watts` 或名称包含 `Pwr Consumption` 的功耗读数计算百分比。自动策略运行时普通传感器轮询不再额外读取 SDR；该 tick 的 SDR 结果负责刷新传感器、图表和历史点。若计算出的百分比与同一自动模式下上次成功下发值不同，最终发送全部风扇百分比命令；若一致，本轮记录 `SkipUnchangedFanPercent` 并显示“未下发风扇命令”，不重复发送相同 raw 风扇控制命令。
...
- Dell 风扇控制 raw 命令（`raw 0x30 0x30 ...`）非零退出码会立即失败；每次实际 `ipmitool` 子进程只写入一次真实结果，不写入 `attempt`、`maxAttempts` 或 `willRetry`，stdout/stderr 会直接显示给用户。`sdr elist` 轮询失败不会重试同一条失败命令，也不会写入伪造历史点；只有后台传感器轮询失败会追加一次可见的真实重连。
...
- 若发布可直接运行的 exe 目录，使用 `tools/Publish-UnpackagedExe.ps1` 或等价命令，并确认输出目录包含 `DellR730xdFanControlCenter.exe`、`Assets/AppIcon.ico`、图表资产和内置 `ipmitool.exe`；该模式需要分发整个目录，不能只复制单个 exe。不要把 `bin\Release\...\publish` 中的 MSIX 构建中间 exe 当作免安装发行物。
- 若发布 GitHub Actions 或 Release 下载 zip，使用 `tools/Publish-ReleaseZip.ps1` 或 `.github/workflows/release.yml`，并确认 zip 解压后包含 exe、`Microsoft.WindowsAppRuntime.dll`、`Microsoft.ui.xaml.dll`、`DellR730xdFanControlCenter.pri`、图表资产和内置 `ipmitool.exe`；本机可用 `tools/Publish-ReleaseZip.ps1 -VerifyLaunch` 验证解压后的 zip 能启动窗口。该 zip 是 unsigned unpackaged 发行物，不应包含 `.msix`、`.pfx`、`.cer`、`AppxManifest.xml` 或 `Package.appxmanifest`；脚本检测到这些签名包或包身份文件时必须失败停止。GitHub Actions workflow 不应调用 `tools\Publish-SignedMsix.ps1`、`Add-AppxPackage` 或 `Get-AuthenticodeSignature`。GitHub Actions 手动触发只上传 workflow artifact，推送 `v*` tag 时才上传 GitHub Release 资产，并用 `gh release upload --clobber` 允许同一 tag 重跑覆盖同名 zip。
- 若发布 MSIX，使用 `tools/Publish-SignedMsix.ps1` 或等价命令签名；脚本必须以 `WindowsAppSDKSelfContained=true` 发布，签名证书 Subject 必须与 `Package.appxmanifest` 的 `Publisher` 完全一致，生成后 `Get-AuthenticodeSignature` 必须返回 `Valid`。签名后还必须拆包确认生成的 `AppxManifest.xml` 不包含外部 `PackageDependency`，并确认包内存在 `Microsoft.WindowsAppRuntime.dll`、`Microsoft.ui.xaml.dll`、图表资产和内置 `ipmitool.exe`。脚本应把 MSIX publish 中间目录放在 `obj\signed-msix\publish` 并在完成后清理，避免留下会被误运行的 `bin\Release\...\publish` 副产物。
