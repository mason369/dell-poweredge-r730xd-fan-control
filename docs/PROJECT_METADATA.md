...
- 已经运行的自动策略在初始 SDR 读取失败，或正常 raw 风扇命令成功后的确认刷新失败时，会显示并记录真实失败，但保留计时器、当前自动模式和 LastRunning 状态，下一次计划 tick 重新读取 SDR；首轮启动失败、raw 命令失败和日志写入失败会停止软件自动。若进入手动模式成功、百分比命令失败，随后一次 Dell 自动恢复确认成功时，本轮仍保持失败，但界面和持久化状态会显示 Dell 自动而不是手动；恢复也失败时硬件模式保持未确认。紧急 Dell 自动同样会停止软件自动并保存 Dell 自动状态。
...
- 达到紧急自动阈值时发送 Dell 自动模式命令，而不是继续下发曲线百分比；命令及确认刷新成功后停止软件自动计时器、清空曲线/软件自动状态，并把 Dell 自动保存为当前运行模式。确认刷新失败时同样停止并清理软件自动状态，但会显示真实失败，且不会把未确认的 Dell 自动状态标为成功。
- 功耗曲线缺少功耗读数时会显示失败原因并停止本轮，不会用默认功耗或上一次功耗继续下发。
- 手动全部风扇、单风扇、内置恢复手动预设、Dell 自动、总览/托盘还原戴尔出厂设置转速和停止自动都是新的用户控制意图，会在等待 IPMI 锁前停止软件自动计时器，并清除当前曲线、LastRunning 自动状态和自动目标缓存。删除正在运行的曲线预设、raw 风扇命令失败、日志写入失败和紧急 Dell 自动也会停止软件自动。Dell 自动恢复确认成功时会保存并显示 Dell 自动，同时保留原百分比请求失败；恢复失败时不会把硬件模式标为已确认。
